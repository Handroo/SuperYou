...
- (IBAction)postComment:(id)sender {
    ...
    [self postCommentOnWall:self.commentTextField.text name:appDelegate.masterTabBarViewController.userData.name user_id:appDelegate.masterTabBarViewController.userData.user_id];

    [self.commentTextField setText:@""];
}

-(void)postCommentOnWall:(NSString*)text name:(NSString*)name user_id:(NSString*)user_id{
    CommentView* comment = [[CommentView alloc]initWithFrame:CGRectMake(0, 470, self.view.frame.size.width, 140)];
    [comment.xibCommentView.commentText setText:text];
    comment.xibCommentView.nameLabel.text = name;
    comment.xibCommentView.commentorId = user_id;
...
//    //Loop through Json objects, create question objects and add them to our questions array
    for (int i = 0; i < jsonArray.count; i++)
    {
        ...
        if([jsonElement[@"type"] isEqualToString:@"mission_comment"]){
            [self postCommentOnWall:jsonElement[@"comment_text"] name:jsonElement[@"name"] user_id:jsonElement[@"user_id"]];
        }
